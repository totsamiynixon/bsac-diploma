<template>
  <f7-login-screen id="sign-up-screen">
    <f7-view>
      <f7-page login-screen>
        ...
        <f7-list form>
          ...
          <f7-list-item>
            <f7-label>Подтвердите пароль</f7-label>
            <f7-input :value="confirmPassword"
                      @input="confirmPassword = $event.target.value"
                      type="text"
                      placeholder="Подтвердите пароль"></f7-input>
          </f7-list-item>
          <f7-list-button type="submit"
                          @click="onSignin">Зарегистрироваться</f7-list-button>
          <f7-list-button link="/auth/sign-in">Войти</f7-list-button>
          <f7-block-footer>
            <p>@BSACTeam</p>
          </f7-block-footer>
        </f7-list>
...
export default {
  ...
  mounted() {
    this.$f7.loginScreen.open("#sign-up-screen");
  },
  created() {
  ...
  methods: {
    onSignin() {
      this.$store.dispatch("user/signUserUp", {
        ...
        configrmPassword: this.confirmPassword
      });
    }
  }
};
